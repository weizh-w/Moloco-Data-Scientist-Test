...
- the negative value of A or B can inflence the other, add interaction of `A*B` or `A*IB` may help explain more variance. However, adding both interaction forms may cause overfitting
...
The selected model performance is excellent, it expains about 97% variation in variable C. However, there may exist overfitting.

Even the redeuced model still has tons of variables, intuitively, some variables seem not neccessay. For example the interaction term between time and `IA`, time variable will use anyway no matter A takes positive or negative value. Therefore, I would delete some interaction terms regarding `IA` and `IB` and run model selection again.
...
So this time, the model fits the data perfectly without being too complecated, diagnosis plots are reseasonablely acceptable. 
...
This model is more ideal interms of performance and interpretability with all diagnosis plots generally valid. Therefore, I choose it as my final regression mode. The model is written as 
